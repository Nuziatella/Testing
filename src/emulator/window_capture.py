...
HAS_MSS = False def get_window_rect(hwnd: int) -> Tuple[int, int, int, int]:
rect = win32gui.GetClientRect(hwnd)
left, top = win32gui.ClientToScreen(hwnd, (rect[0], rect[1]))
right, bottom = win32gui.ClientToScreen(hwnd, (rect[2], rect[3]))
return left, top, right, bottom def get_top_screen_rect(hwnd: int) -> Tuple[int, int, int, int]:
# Assume melonDS uses two 256x192 screens stacked vertically in client area.
l, t, r, b = get_window_rect(hwnd)
width = r - l
height = b - t
# Heuristic: top screen is top half when displayed vertically
# If aspect not matching, fallback to upper 256x192 centered
...
y = t + 0
return x, y, x + target_w, y + target_h class WindowCapturer:
def init(self, hwnd: int, use_dxcam: bool = True, target_size: Tuple[int, int] = (84, 84)):
self.hwnd = hwnd
